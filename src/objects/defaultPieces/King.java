...
public class King extends Piece
{
	...
	public ArrayList<Point> validMoves()
	{
		...
		if (firstMove && board.checkedTeam()!=team) // castling
		{
			for (int dir = -1; dir <= 1;dir+=2)
			{
				...
				if (board.validLocation(move))
				{
					Piece p = board.pieceAt(move);
					if (p instanceof Rook && ((Rook) p).firstMove)
					{
						if (board.isUncheckedMove(this, new Point(loc.x + dir, loc.y))
								&& board.isUncheckedMove(this,  new Point(loc.x + dir*2, loc.y)))
	...
	public void moveTo(Point newLoc)
	{
		...
		if (newLoc.distance(loc)==2)
		{
			...
			while (!board.hasPieceAt(rook) && board.validLocation(rook))
			{
				i++;
	...
	{
		if (team == 0)
			g.drawImage(ImageHelper.getPicture("pieces/WKing.png"), i, j,
					size, size, null);
		else
			g.drawImage(ImageHelper.getPicture("pieces/BKing.png"), i, j,
					size, size, null);
	}
